...
local keymaps = {
  lsp = {
    n = {
      ...
      ['<leader>k'] = { '<CMD>lua vim.lsp.buf.signature_help()<CR>', '[LSP]: Parameters of a function and their descriptions' },
  ...
  telescope = {
    n = {
      ['<leader>ff'] = { '<cmd>lua require("telescope.builtin").find_files({ previewer = true })<cr>', 'Telescope: Find files' },
      ['<leader>fg'] = { '<cmd>lua require("telescope.builtin").live_grep()<cr>', 'Telescope: Find grep' },
      ['<leader>fb'] = { '<cmd>lua require("telescope.builtin").buffers({ previewer = false })<cr>', 'Telescope: Find buffers' },
